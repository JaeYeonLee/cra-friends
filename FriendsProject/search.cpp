...
const list<EmployeeInfo*> SearchEngine::search(string key, unordered_multimap<string, int>* hashTable, unordered_map<int, EmployeeInfo*>& employeeInfoMap, SearchKey key_type) {
...
const list<EmployeeInfo*> SearchEngine::search(int key, unordered_multimap<int, int>* hashTable, unordered_map<int, EmployeeInfo*>& employeeInfoMap, SearchKey key_type) {
	list<EmployeeInfo*> result;
	if (hashTable->find(key) == hashTable->end()) return result;
	auto range = hashTable->equal_range(key);
	for (auto it = range.first; it != range.second;) {
